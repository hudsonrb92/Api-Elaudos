...
class AnexoEstudoDicomList(Resource):
    ...
    def get(self, identificador_estudo_dicom):
        logger.info("Rota get AnexoEstudoDicomList por identificador_estudo acessada.")
        anexos = anexo_estudo_dicom_service.listar_anexo_por_identificador_estudo(identificador_estudo_dicom)
        logger.info(f"Tem {len(anexos)} anexo(s).")
        logger.info(f"Identificador_estudo_dicom buscado: {identificador_estudo_dicom}")
        if anexos is None:
...
class AnexoEstudoDicomDetail(Resource):
    ...
    def get(self, identificador):
        ...
        anexo = anexo_estudo_dicom_service.listar_anexo_por_identificador(identificador)
...
api.add_resource(AnexoEstudoDicomList, '/api/anexos/<int:identificador_estudo_dicom>')
